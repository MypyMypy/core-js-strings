...
function concatenateStrings(value1, value2) {
  const trimValue = value1.trim();
  const trimValue2 = value2.trim();

  if (trimValue.length > trimValue2.length) {
    return value1;
  }
  if (trimValue2.length > trimValue.length) {
    return value2;
  }
 ...
 */
function removeFirstOccurrences(s, subs) {
  const indexSubs = s.indexOf(subs);
  if (indexSubs === -1) return s;
  const start = s.substring(0, indexSubs);
  const end = s.substring(indexSubs + subs.length);
  return start + end;
}
 ...
 */
function removeLastOccurrences(s, subs) {
  const lastIndexSubs = s.lastIndexOf(subs);
  if (lastIndexSubs === -1) return s;
  const start = s.substring(0, lastIndexSubs);
  const end = s.substring(lastIndexSubs + subs.length);
  return start + end;
}
 ...
 */
function containsSubstring(s, subs) {
...
function countVowels(s) {
  ...
  return Array.from(s).reduce((total, char) => {
    if (vowels.includes(char)) return total + 1;
    return total;
  }, 0);
}
...
function isPalindrome(s) {
  const fs = s
    ...
    .replace(/[ ,?!]+/g, '')
    .toLowerCase();
  const reverseFs = fs.split('').reverse().join('');
  return fs === reverseFs;
}
...
function findLongestWord(s) {
  if (s) {
    return s.split(' ').reduce((total, item) => {
      if (total.length < item.length) return item;
      return total;
    }, '');
  }
  return s;
 ...
 */
function getStringFromTemplate(fn, sn) {
  return 'Hello, '.concat(fn, ' ', sn, '!');
}
...
function unbracketTag(s) {
  return s.substr(s.indexOf('<') + 1, s.lastIndexOf('>') - 1);
}
  ...
  return s
    ...
    .map((char) => {
      if (/[a-zA-Z]/.test(char)) {
        ...
        const encodedChar = String.fromCharCode(alphabetStart + offset);
